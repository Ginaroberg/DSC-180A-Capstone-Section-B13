...
def get_test_data(file, eof_solvers, n_eofs=5):
    ...
    # Create training and testing arrays
    X = xr.open_dataset(data_path_test + f"inputs_{file}.nc")
...
def create_predictdand_data_test(data_sets):
    if isinstance(data_sets, str):
        data_sets = [data_sets]
    Y = xr.concat([xr.open_dataset(data_path_test + f"outputs_{file}.nc") for file in data_sets], dim='time').mean("member")
    # Convert the precip values to mm/day
    Y["pr"] *= 86400
    Y["pr90"] *= 86400
    return Y
